...
class WarframeModding extends Component {
  ...
  decryptPre = () => {
    ...
    let totalModsCost = this.calcCost(
      ...
      prePolarities.aura,
      preMods.exilusMod,
      prePolarities.exilus
    );
    let formaCount = this.countForma(
      prePolarities.stack,
      prePolarities.aura,
      prePolarities.exilus
    );
    let modSets = this.checkModSets(
      preMods.auraMod,
      preMods.exilusMod,
      preMods.chosenMods
    );
  ...
  convertBuildToString = (
    ...
    buildStr += this.convertPolarityToNum(auraPolarity);
    buildStr += this.convertPolarityToNum(exilusPolarity);
    for (let i = 0; i < 8; i++) {
      let polNum = this.convertPolarityToNum(slotPolarities[i]);
      buildStr += polNum;
    }
    buildStr += this.convertModSlotToString(chosenAuraMod);
    buildStr += this.convertModSlotToString(chosenExilusMod);
    chosenMods.forEach(mod => {
      buildStr += this.convertModSlotToString(mod);
    });
    buildStr += this.convertModSlotToString(arcanes[0]);
    buildStr += this.convertModSlotToString(arcanes[1]);
  ...
  autoForma = () => {
    try {
      ...
      let auraPolarity = this.state.forceNeutAura
        ? undefined
        : this.state.forceCurrAura
        ? this.state.auraPolarity
        : this.props.frame.aura;
      let exilusPolarity = this.state.forceNeutExilus
        ? undefined
        : this.state.forceCurrExilus
        ? this.state.exilusPolarity
        : this.props.frame.exilus;
      let totalModsCost = this.calcCost(
        this.state.chosenMods,
        arrangedOriginals,
        this.state.chosenAuraMod,
        auraPolarity,
        this.state.chosenExilusMod,
        exilusPolarity
      );
      if (cap - totalModsCost >= 0) {
        ...
        let mismatch = this.formaMismatch(
          auraPolarity,
          exilusPolarity,
          arrangedOriginals,
          cap
        );
        totalModsCost = this.calcCost(
          this.state.chosenMods,
          mismatch.slots,
          this.state.chosenAuraMod,
          mismatch.aura,
        ...
        if (cap - totalModsCost >= 0) {
          ...
          let finalPolarities = this.calcAutoForma(
            mismatch.aura,
            mismatch.exilus,
            mismatch.slots,
            cap
          ...
          totalModsCost = this.calcCost(
            this.state.chosenMods,
            finalPolarities.slots,
            this.state.chosenAuraMod,
            finalPolarities.aura,
            this.state.chosenExilusMod,
            finalPolarities.exilus
          );
          this.setAutoForma(
            finalPolarities.aura,
            finalPolarities.exilus,
  ...
  formaMismatch = (auraPolarity, exilusPolarity, slotPolarities, cap) => {
    ...
    if (mismatches.length > 0) {
      ...
      if (
        this.state.chosenAuraMod.name &&
        this.state.chosenAuraMod.polarity !== auraPolarity &&
        !this.state.forceNeutAura &&
        !this.state.forceOriginalAura &&
        !this.state.forceCurrAura
      ) {
        withPolAura = this.calcCost(
          this.state.chosenMods,
          slotPolarities,
          this.state.chosenAuraMod,
          this.state.chosenAuraMod.polarity,
          this.state.chosenExilusMod,
          exilusPolarity
        );
      }
      if (
        this.state.chosenExilusMod.name &&
        this.state.chosenExilusMod.polarity !== exilusPolarity &&
        !this.state.forceNeutExilus &&
        !this.state.forceOriginalExilus &&
        !this.state.forceCurrExilus
      ) {
        withPolExilus = this.calcCost(
          this.state.chosenMods,
          slotPolarities,
          this.state.chosenAuraMod,
          auraPolarity,
          this.state.chosenExilusMod,
          this.state.chosenExilusMod.polarity
        );
      }
      this.state.chosenMods.forEach((mod, index) => {
        if (
      ...
      if (mismatches.length > 0 && leftoverCap < 0) {
        postMismatch = this.formaMismatch(
          postMismatch.aura,
          postMismatch.exilus,
          postMismatch.slots,
          cap
  ...
  calcAutoForma = (auraPolarity, exilusPolarity, slotPolarities, cap) => {
    ...
    if (leftoverCap < 0) {
      if (
        this.state.chosenAuraMod.name &&
        this.state.chosenAuraMod.polarity !== finalPolarities.aura &&
        !this.state.forceNeutAura &&
        !this.state.forceOriginalAura &&
        !this.state.forceCurrAura
      ) {
        ...
        this.state.chosenExilusMod.name &&
        this.state.chosenExilusMod.polarity !== finalPolarities.exilus &&
        !this.state.forceNeutExilus &&
        !this.state.forceOriginalExilus &&
        !this.state.forceCurrExilus
      ) {
        ...
        this.state.chosenMods.forEach((mod, index) => {
          if (
            mod.name &&
            mod.polarity !== finalPolarities.slots[index] &&
            mod.polarity !== "umbra"
          ) {
            available = true;
          }
      ...
      if (available) {
        finalPolarities = this.calcAutoForma(
          finalPolarities.aura,
          finalPolarities.exilus,
          finalPolarities.slots,
          cap
  ...
  pickMod = mod => {
    ...
    if (typeof this.state.forSlot === "number") {
      ...
      if (sameFamilySlot === -1) {
        ...
        let totalModsCost = this.calcCost(
          chosenMods,
          this.state.slotPolarities,
          this.state.chosenAuraMod,
          this.state.auraPolarity,
          this.state.chosenExilusMod,
          this.state.exilusPolarity
        );
        let modsSets = this.checkModSets(
          this.state.chosenAuraMod,
          this.state.chosenExilusMod,
          chosenMods
        );
    ...
    if (mod.aura || targetSlot === "aura") {
      if (mod.aura && targetSlot === "aura") {
        if (this.state.chosenAuraMod.name) {
          chosenIndexs = chosenIndexs.filter(
            idx => idx !== this.state.chosenAuraMod.index
          );
        }
        chosenIndexs.push(mod.index);
        let totalModsCost = this.calcCost(
      ...
      if (mod.exilus) {
        if (this.state.chosenExilusMod.name) {
          chosenIndexs = chosenIndexs.filter(
            idx => idx !== this.state.chosenExilusMod.index
          );
        }
        chosenIndexs.push(mod.index);
        let totalModsCost = this.calcCost(
      ...
      let sameFamilySlot = -1;
      if (mod.family) {
        sameFamilySlot = this.state.chosenMods.findIndex(
          (slottedMod, index) => {
  ...
  calcCost = (
    ...
    if (auraMod.name) {
      if (auraMod.polarity === auraPolarity) {
        ...
        modsCostSum -= Math.round(
          (auraMod.baseCost + auraMod.currRank) * 0.725
        );
      }
    }
    if (exilusMod.name) {
  ...
  swapMods = (startSlot, targetSlot) => {
    if (
      ...
      if (typeof startSlot === "number" && typeof targetSlot === "number") {
        ...
        targetSlot === "exilus" &&
        this.state.chosenMods[startSlot].exilus
      ) {
        let mods = cloneDeep(this.state.chosenMods);
        let temp = this.state.chosenMods[startSlot];
        ...
        startSlot === "exilus" &&
        (this.state.chosenMods[targetSlot].exilus ||
          !this.state.chosenMods[targetSlot].name)
      ) {
        let mods = cloneDeep(this.state.chosenMods);
      ...
      return (
        <div className="message-wrapper always-on">
          {this.state.forSwap !== "exilus" ? (
            ...
            <p className="display-message">
              Choose a mod slot to swap with {this.state.chosenExilusMod.name}.
            </p>
          )}
        </div>
      );
      ...
      >
            ...
            <div className="special-slots">
              ...
              >
                <ModStateHandler
                  mod={chosenAuraMod}
                  slot={"aura"}
                  slotPolarity={auraPolarity}
                  forma={forma}
                  openModPicker={this.openModPicker}
                  removeMod={this.removeAura}
              ...
              >
                <ModStateHandler
                  mod={chosenExilusMod}
                  slot={"exilus"}
                  slotPolarity={exilusPolarity}
                  forma={forma}
                  openModPicker={this.openModPicker}
                  removeMod={this.removeExilus}
                  handleRankUpdate={this.handleRankUpdate}
                  showPolarityPicker={this.showPolarityPicker}
                  forSwap={forSwap}
                  startSwap={this.startSwap}
        ...
        <div className="modding-right">
          <WarframeStats
            ...
            full={true}
            mods={chosenMods}
            chosenExilusMod={chosenExilusMod}
            chosenAuraMod={chosenAuraMod}
            viewWidth={this.props.viewWidth}
          />
          ...
          <div className="autoforma-box">
            ...
            <div className="autoforma-options">
              ...
              <div className="autoforma-option">
                ...
                <Switch
                  onChange={this.forceNeutAura}
                  checked={this.state.forceNeutAura}
                />
              </div>
              <div className="autoforma-option">
                <p>Force current Aura slot</p>
                <Switch
                  onChange={this.forceCurrAura}
                  checked={this.state.forceCurrAura}
                />
              </div>
              <div className="autoforma-option">
                <p>Force original Aura slot</p>
                <Switch
                  onChange={this.forceOriginalAura}
                  checked={this.state.forceOriginalAura}
                />
              </div>
              <div className="autoforma-option">
                <p>Force neutral Exilus slot</p>
                <Switch
                  onChange={this.forceNeutExilus}
                  checked={this.state.forceNeutExilus}
                />
              </div>
              <div className="autoforma-option">
                <p>Force current Exilus slot</p>
                <Switch
                  onChange={this.forceCurrExilus}
                  checked={this.state.forceCurrExilus}
                />
              </div>
              <div className="autoforma-option">
                <p>Force original Exilus slot</p>
                <Switch
                  onChange={this.forceOriginalExilus}
                  checked={this.state.forceOriginalExilus}
                />
              </div>
            </div>
