...
class MeleeModding extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.match.params.pre) {
      ...
      let exaltedStance = this.props.mods[
        this.props.mods.findIndex(mod => {
          return (
            mod.name.toLowerCase() === this.props.item.type[0].toLowerCase()
          );
        })
      ];
  ...
  decryptPre = () => {
    ...
    let preArcanes =
      this.props.type === "zaws"
        ? build[46] === "x"
          ? this.createPreArcanes(build.slice(80, 84))
          : this.createPreArcanes(build.slice(47, 51))
        : [{}];
    let totalModsCost = this.calcCost(
      preMods.chosenMods,
      prePolarities.stack,
      preMods.stanceMod,
      prePolarities.stance
    );
  ...
  convertBuildToString = (
    ...
    buildStr += this.convertPolarityToNum(stancePolarity);
    for (let i = 0; i < 8; i++) {
      let polNum = this.convertPolarityToNum(slotPolarities[i]);
      buildStr += polNum;
    }
    chosenStanceMod.name
      ? (buildStr += this.convertModSlotToString(chosenStanceMod))
      : (buildStr += "0000");
    ...
    if (this.props.type === "zaws" && arcanes[0].name)
  ...
  toggleForma = () => {
    if (this.state.forma) {
      ...
      if (this.props.item.name === "PARACESIS") {
        cap = this.state.orokin
          ? this.state.formaCount < 6
            ? 60 + 4 * this.state.formaCount
            : 80
          : this.state.formaCount < 6
          ? 30 + 2 * this.state.formaCount
          : 40;
  ...
  resetBuild = () => {
    if (this.props.match.params.pre) {
      ...
      let stanceMod = {};
      let chosenIndexes = [];
      if (this.props.item.exalted && this.props.item.name !== "DIWATA") {
        stanceMod = this.props.mods[
          this.props.mods.findIndex(mod => {
            return (
              mod.name.toLowerCase() === this.props.item.type[0].toLowerCase()
            );
      ...
      this.setState({
        ...
        slotPolarities: this.props.slotPolarities,
        forSlot: null,
        chosenStanceMod: stanceMod,
        chosenMods: [{}, {}, {}, {}, {}, {}, {}, {}],
        chosenIndexs: chosenIndexes,
        totalModsCost: 0,
        polarityPicker: false,
        modPicker: false,
        errorBlinker: null,
  ...
  autoForma = () => {
    try {
      ...
      let stancePolarity = this.state.forceNeutStance
        ? undefined
        : this.state.forceCurrStance
        ? this.state.stancePolarity
        : this.props.item.stance;
      let totalModsCost = this.calcCost(
        this.state.chosenMods,
        arrangedOriginals,
        this.state.chosenStanceMod,
        stancePolarity
      );
  ...
  formaMismatch = (stancePolarity, slotPolarities, cap) => {
    ...
    if (mismatches.length > 0) {
      ...
      if (
        this.state.chosenStanceMod.name &&
        this.state.chosenStanceMod.polarity !== stancePolarity &&
        !this.state.forceNeutStance &&
        !this.state.forceOriginalStance &&
        !this.state.forceCurrStance
      ) {
        withPolStance = this.calcCost(
          this.state.chosenMods,
          slotPolarities,
          this.state.chosenStanceMod,
          this.state.chosenStanceMod.polarity
        );
      }
      this.state.chosenMods.forEach((mod, index) => {
        if (
  ...
  calcAutoForma = (stancePolarity, slotPolarities, cap) => {
    ...
    if (leftoverCap < 0) {
      if (
        this.state.chosenStanceMod.name &&
        this.state.chosenStanceMod.polarity !== finalPolarities.stance &&
        !this.state.forceNeutStance &&
        !this.state.forceOriginalStance &&
        !this.state.forceCurrStance
      ) {
      ...
      if (mod.stance && targetSlot === "stance") {
        if (this.state.chosenStanceMod.name) {
          chosenIndexs = chosenIndexs.filter(
            idx => idx !== this.state.chosenStanceMod.index
          );
        }
        chosenIndexs.push(mod.index);
        let totalModsCost = this.calcCost(
    ...
    if (stanceMod.name) {
      if (stanceMod.polarity === stancePolarity) {
        ...
        modsCostSum -= Math.round(
          (stanceMod.baseCost + stanceMod.currRank) * 0.725
        );
      }
    }
  ...
  hasChanged = currStr => {
    if (this.props.match.params.pre) {
      ...
      let stanceMod = this.props.item.exalted ? this.state.chosenStanceMod : {};
      let untouched = this.convertBuildToString(
        true,
        this.props.stancePolarity,
        stanceMod,
                    ...
                    <React.Fragment>
                      {orokin ? (
                        <p
                          ...
                          style={
                            formaCount < 6
                              ? 60 + 4 * formaCount - totalModsCost >= 0
                                ? { color: "#15E610" }
                                : { color: "red" }
                              : 80 - totalModsCost >= 0
                              ? { color: "#15E610" }
                        ...
                        <p
                          ...
                          style={
                            formaCount < 6
                              ? 30 + 2 * formaCount - totalModsCost >= 0
                                ? { color: "#15E610" }
                                : { color: "red" }
                              : 40 - totalModsCost >= 0
                              ? { color: "#15E610" }
              ...
              <div className="special-slots">
                ...
                >
                  <ModStateHandler
                    mod={chosenStanceMod}
                    slot={"stance"}
                    slotPolarity={stancePolarity}
                    item={this.props.item}
                    forma={forma}
                    openModPicker={this.openModPicker}
                    removeMod={this.removeStance}
          ...
          <div className="autoforma-box">
            ...
            <div className="autoforma-options">
              ...
              <div className="autoforma-option">
                ...
                <Switch
                  onChange={this.forceNeutStance}
                  checked={this.state.forceNeutStance}
                />
              </div>
              <div className="autoforma-option">
                <p>Force current Stance slot</p>
                <Switch
                  onChange={this.forceCurrStance}
                  checked={this.state.forceCurrStance}
                />
              </div>
              <div className="autoforma-option">
                <p>Force original Stance slot</p>
                <Switch
                  onChange={this.forceOriginalStance}
                  checked={this.state.forceOriginalStance}
                />
              </div>
            </div>
