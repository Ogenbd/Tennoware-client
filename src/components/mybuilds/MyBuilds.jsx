...
const updateTimesImport = () =>
  import("../../data/updatetimes" /* webpackChunkName: "upst" */);
    ...
    if (builds.length > 0) {
      await Promise.all(
        builds.map(async build => {
          ...
          if (
            build.Type === "primaryweapons" ||
            build.Type === "secondaryweapons" ||
            build.Type === "sentinelweapons" ||
            build.Type === "meleeweapons" ||
            build.Type === "kitguns"
          )
            arsenalItem.hasRivens = true;
          ...
          if (
            updateTimeIndex === -1 ||
            updateTimes[updateTimeIndex].updated < arsenalItem.date
          ) {
  ...
  getItemImage = async build => {
    ...
    if (build.Type === "moas" || build.Type === "kitguns") {
      itemIdx = list.first.findIndex(item => {
        return (
          item.name.toLowerCase() === build.ItemName.toLowerCase().split(" ")[0]
        );
      });
  ...
  generateList = () => {
    ...
    this.state.arsenal.forEach((item, index) => {
      if (
        ...
        item.buildName.toLowerCase().includes(this.state.search.toLowerCase())
      ) {
        let date = new Date(item.date).toLocaleDateString();
        let potato;
        if (
          item.type === "primaryweapons" ||
          item.type === "secondaryweapons" ||
          item.type === "sentinelweapons" ||
          item.type === "meleeweapons" ||
          item.type.includes("archguns") ||
          item.type === "archmelees" ||
          item.type === "zaws" ||
          item.type === "kitguns"
        ) {
        ...
        item.orokin === 1
          ? (orokinStr = potato)
          : (orokinStr = require("../../assets/general/nocatalyst.png"));
        item.riven === 1
          ? (riven = require("../../assets/general/rivenon.png"))
          : (riven = require("../../assets/general/rivenoff.png"));
        let itemBlock = (
          <div key={index} className="my-build-container">
            ...
            <div className="my-build-list-item">
              ...
              <div className="my-build-item-row info-row">
                <img
                  className="my-build-list-img"
                  src={orokinStr}
                  alt={"orokin"}
                />
                <div className="my-build-list-forma-block">
                  <img
                    className="my-build-list-img"
                ...
                {item.hasRivens ? (
                  ...
                  <div className="my-riven-placeholder" />
                )}
              </div>
            </div>
            <div className="my-build-buttons">
              <Link
                to={`/${this.state.arsenal[index].type}/${
                  this.state.arsenal[index].name
                }/${this.state.arsenal[index].buildStr}/${
                  this.state.arsenal[index].id
                }`}
              >
                <div className="interactable interactable-semi-inactive">
                  <p className="interactable-p">Open</p>
                </div>
              </Link>
              {item.source === "builds" ? (
                <div
                  className="interactable interactable-semi-inactive delete-button"
                  onClick={() => {
        ...
        item.source === "builds"
          ? saved.push(itemBlock)
          : liked.push(itemBlock);
      }
    });
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <div className="screen">
          ...
          <div className="my-builds">
            ...
            <div
              ...
              className="builds-wrapper"
              style={this.state.ready ? { opacity: 1 } : { opacity: 0 }}
            >
              {this.props.online && <BottomAd />}
                ...
                <React.Fragment>
                  {this.state.arsenal.length < 1 ? (
                    <div className="no-builds">
                      You have not saved or liked any builds yet.
                    </div>
                  ) : (
                    <React.Fragment>
                      {list.saved.length > 0 && (
                      ...
                      {list.liked.length > 0 && (
                        <React.Fragment>
                          <div className="my-builds-subtitle">
                            Bookmarked Builds
                          </div>
                          {list.liked}
                        </React.Fragment>
              ...
              <div className="conformation-box">
                ...
                <div className="conformation-buttons">
                  <div
                    ...
                    onClick={this.unsetRemoval}
                  >
                    <p className="interactable-p">Cancel</p>
                  </div>
                  <div
                    className="interactable interactable-semi-inactive delete-button"
                    onClick={this.unlikeBuild}
                  >
                    <p className="interactable-p">Unbookmark</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        {(!this.state.ready || this.state.getting) && <Loading />}
      </React.Fragment>
    );
  }
}
