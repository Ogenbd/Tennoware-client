    ...
    setupBuilder = async (metaInfo) => {
        setTimeout(() => {
            Promise.all([this.props.items(), this.props.mods()]).then(data => {
                let item = data[0].default.find(pet => {
                    return pet.name.toLowerCase() === decodeURIComponent(this.props.match.params.id.toLowerCase());
                });
                if (item !== undefined) {
                    let slotPolarities = [];
                    ...
                    let filteredMods = data[1].default.filter(mod => {
                        return mod.type === 'COMPANION' || mod.type === 'BEAST' || item.type.includes(mod.type) || mod.type === 'ALL';
                    });
