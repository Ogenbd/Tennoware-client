        ...
        setTimeout(() => {
            Promise.all([this.props.items(), this.props.mods()]).then(data => {
                let item = data[0].default.find(archwing => {
                    return archwing.name.toLowerCase() === decodeURIComponent(this.props.match.params.id.toLowerCase());
                });
                if (item !== undefined) {
                    let slotPolarities = [];
                ...
                {this.state.error !== null
                    ...
                    : <React.Fragment>
                        {this.state.item.name &&
                            <EightSlotModding redirectToVoid={this.redirectToVoid} updateInfo={this.updateInfo} readyToShow={this.readyToShow} type={this.props.type} orokin={require('../assets/general/reactor.png')} item={this.state.item} mods={this.state.relevantMods} slotPolarities={this.state.slotPolarities} originalPolarityCount={this.state.originalPolarityCount} viewWidth={this.props.viewWidth} match={this.props.match} user={this.props.user} metaInfo={this.state.metaInfo} online={this.props.online} />
                        }
                        {this.state.loading &&
                            <Loading />
                        }
